...
sed -i "/submission/ s|#||
  /smtps/ s|#||
  /syslog_name=/ s|#||
  /smtpd_tls_security_level=/ s|#|| 
  /smtpd_tls_wrappermode=/ s|#||
  /smtpd_reject_unlisted_recipient=/ s|#||
  /smtpd_relay_restrictions=/ s|#||
  /milter_macro_daemon_name=/ s|#||" /etc/postfix/master.cf

# Update master.cf for Dovecot's Vmail support.
grep -q "dovecot" /etc/postfix/master.cf || \
